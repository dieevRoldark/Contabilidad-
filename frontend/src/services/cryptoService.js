...
let clavePublicaServidorCache = null
...
export async function cifrarHibridoConClave(datos, publicKey) {
    const subtle = obtenerSubtleCrypto()
    const claveAES = await generarClaveAES()
    const texto = typeof datos === 'string' ? datos : JSON.stringify(datos)
    const iv = crypto.getRandomValues(new Uint8Array(12))
...
export async function obtenerClavePublicaServidor() {
    if (clavePublicaServidorCache) {
        return clavePublicaServidorCache
    }

    const subtle = obtenerSubtleCrypto()
    const respuesta = await fetch(`${globalThis.API_HOST || ''}/api/security/public-key`)

    if (!respuesta.ok) {
        throw new Error('No se pudo obtener la clave publica del servidor.')
    }

    const jwk = await respuesta.json()

    clavePublicaServidorCache = await subtle.importKey(
        'jwk',
        jwk,
        {
            name: 'RSA-OAEP',
